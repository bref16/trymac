# -*- coding: utf-8 -*-
"""
MAG Config — DB-backed panel with right-side actions panel (QSplitter).

- Modes (top bar) are read from DB table 'Modes' (column 'Mode'), not hard-coded.
- Right header buttons (in Russian): "Редактировать значения/таблицы", "Настроить подключение к БД" (switches to "Подключено").
- Quick "Подключиться" button in the actions grid (also switches to "Подключено" upon success).
- Left panel lists components filtered by current Mode (and F side), quantity spinbox, add-to-summary.
- Summary table shows PN, Description, Qty, Pack, List Price ("Стоимость"), TRIMM price (displayed as base*Logistics).
- Pricing controls: Discount %, Logistics, Exchange rate; live Total and Margin.
- Template buttons (EVE TR/NEO/IN/ALL, S, F лев/прав) populate with quantities from Templates.Qts.
- KP Export writes first item at row 5 (B/C/D/E, F=E*D), others at rows 22,23,... ; uses template.xlsx beside the app.
- Removed TX-related buttons and the old bottom "Export to Excel" row button (as requested).
"""
...
MODES_TABLE = 'Modes'      # read buttons from this table
MODES_COL = 'Mode'
...
class Panel(QtWidgets.QWidget):
    ...
    COL_CTRL = 4
    COL_PACK = 5
    COL_PRICE_LIST = 6       # 'Цена Лист 25' per unit
    COL_PRICE_TRIMM = 7      # 'Цена ТРМ 25 Спец' * Logistics (display); base stored in UserRole

    USERROLE_BASE_TRIMM = QtCore.Qt.UserRole + 1   # base (without logistics)
    USERROLE_LIST_PRICE = QtCore.Qt.UserRole + 2   # per-unit list price
    ...
    def __init__(self):
        ...
        self.available_modes: List[str] = ["EVE", "S", "F"]  # will be replaced from DB on connect
        ...
        # tin_index: key PN -> (desc, pack, price_list25, price_trimm25)
        ...
        # price columns
        self.tin_price_list_col = None         # 'Цена Лист 25'
        self.tin_price_trimm_col = None        # 'Цена ТРМ 25 Спец'

        # templates: normalized type -> list[(pn, qty)]
        self.templates: Dict[str, List[Tuple[str, int]]] = {}

        self._building_table = False  # guard for itemChanged loops
        self._build_ui()

    # ------------------------------ UI ------------------------------
    def _build_ui(self):
        # Hidden connection fields (edited via dialog)
        ...
        # Header (single line): modes left, admin/connect right
        ...
        # Right column container (actions + summary)
        ...
        # Controls: Скидка %, Логистика, Курс
        ...
        totals.addWidget(self.lbl_margin)
        totals.addStretch(1)
        rlay.addLayout(totals)

        # react to controls
        self.discountSpin.valueChanged.connect(self._recalc_totals)
        self.kursSpin.valueChanged.connect(self._recalc_totals)
        self.logisticsSpin.valueChanged.connect(self._on_logistics_changed)
        ...
        # Put left & right into a QSplitter to decouple sizing
    ...
    def _make_actions_box(self) -> QtWidgets.QGroupBox:
        """Top-right panel with action buttons (3-column grid)."""
        ...
        # Row 3
        add_btn("EVE ALL", 3, 0, lambda: self.set_mode("EVE"))
        # Row 4
    ...
    def _load_modes(self):
        """Populate self.available_modes from DB table 'Modes' ('Mode' column)."""
    ...
    def _preload_all_db(self):
        ...
        with self.engine.connect() as conn:
            ...
            # preload TIN index (desc, pack, list, trimm)
    ...
    def _apply_template(self, type_name: str):
        ...
        added = 0
        for pn, qty in items:
            added += self._add_row_by_pn(pn, qty)
        if added:
            self._renumber()
            self.status.setText(f'Добавлено из "{type_name}": {added} позиций.')
        else:
...
def _fetch_tin_by_pn(self, pn: str):
    ...
    def _on_logistics_changed(self, _val: float):
        # recompute per-row displayed TRIMM using stored base
        self._building_table = True
        try:
    ...
    def _export_kp(self):
        ...
        # collect rows (PN, Desc, Qty, Unit List Price)
        ...
        def write_line(row_index: int, rec: Tuple[str, str, int, float]):
            ...
            ws.cell(row=row_index, column=2, value=pn)            # B
            ws.cell(row=row_index, column=3, value=desc)          # C
            ws.cell(row=row_index, column=4, value=qty)           # D
            ws.cell(row=row_index, column=5, value=unit_price)    # E
            ws.cell(row=row_index, column=6, value=f"=E{row_index}*D{row_index}")  # F

        write_line(5, items[0])  # first row at 5
        start_row = 22
        ...
        QtWidgets.QMessageBox.information(
            self, "Выгрузить КП",
            f"Готово: записано {len(items)} позиций в файл:\n{out_path}"
        )
    ...
    def _launch_admin_tool(self):
        """Open the DB editor (Tk app). Works in source and in frozen builds."""
        import sys, os, subprocess

        def base_dir():
            # folder where the running app lives
            return os.path.dirname(sys.executable) if getattr(sys, "frozen", False) \
                   else os.path.dirname(os.path.abspath(__file__))

        # pass current DB creds to the editor
        env = os.environ.copy()
        env.update({
        ...
        base = base_dir()
        exe_name = "pg_admin_gui.exe" if os.name == "nt" else "pg_admin_gui"

        # Built app: prefer onedir subfolder first; Dev mode: .py
        if getattr(sys, "frozen", False):
            candidates = [
                os.path.join(base, "pg_admin_gui", exe_name),  # dist/MAGConfig/pg_admin_gui/pg_admin_gui.exe
                os.path.join(base, exe_name),                  # next to MAGConfig.exe
            ]
        else:
            candidates = [
                os.path.join(base, "pg_admin_gui.py"),         # next to mag_config.py
                os.path.join(os.path.dirname(base), "pg_admin_gui.py"),
            ]
            ...
            if os.path.exists(path):
                try:
                    ...
                    QtWidgets.QMessageBox.critical(
                        self, "Редактировать значения/таблицы",
                        f"Не удалось запустить редактор:\n{e}"
                    )
                    return

        QtWidgets.QMessageBox.critical(
            self, "Редактировать значения/таблицы",
            "Редактор БД не найден. Поместите папку 'pg_admin_gui\\' (onedir сборка) "
            "или файл 'pg_admin_gui.exe' рядом с MAGConfig.exe."
        )
